...
class MainViewModel : ViewModel() {
    ...
    fun createNewPlaylist() {
        ...
        viewModelScope.launch {
            when (val result = networkManager.createPlaylist()) {
                is Result.Success -> {
                    _tracks.value = result.data.sortedBy { it.albumName }
                }
                is Result.Error -> {
    ...
    fun addAlbumTracks(trackDto: TrackDto) {
        _loading.value = true
        viewModelScope.launch {
            when (val result = networkManager.addAlbumTracks(trackDto.albumId)) {
                is Result.Success -> {
                    ...
                    _tracks.value = _tracks.value?.let { tracks ->
                        (tracks + resultWithAlbumData).sortedBy { it.albumName }
                    }
                }
                is Result.Error -> {
                    _errors.value = result.errorMessage
