...
class TracksListAdapter(
    private val onAddClicked: (trackDto: TrackDto) -> Unit,
    ...
    inner class TrackViewHolder(private val binding: TrackItemBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
            binding.addAlbumTracksButton.setOnClickListener {
                items.getOrNull(adapterPosition)?.let { trackDto -> onAddClicked(trackDto) }
            }
